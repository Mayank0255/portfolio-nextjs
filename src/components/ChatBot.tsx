...
export function ChatBot() {
  ...
  return (
    <>
      ...
      {/* Floating Button - always visible on desktop, hidden on mobile when chat is open */}
      <button
        onClick={() => { setIsOpen(!isOpen); if (isOpen) setIsExpanded(false); }}
        className={`cursor-pointer fixed bottom-4 right-4 md:right-8 z-50 w-14 h-14 rounded-full bg-[var(--link-color)] text-white shadow-lg hover:shadow-xl hover:scale-105 transition-all flex items-center justify-center ${isOpen && isExpanded ? "hidden" : isOpen ? "max-md:hidden" : ""}`}
